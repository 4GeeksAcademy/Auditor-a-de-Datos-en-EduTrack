...
-- Comprobación previa: SELECT * FROM enrollments WHERE instructor IS NULL;
UPDATE enrollments
...
-- Comprobación previa: SELECT * FROM enrollments WHERE student_email LIKE '%@test.com';
...
SELECT category, SUM(monthly_fee_paid) AS total
FROM enrollments
GROUP BY category
ORDER BY total DESC;